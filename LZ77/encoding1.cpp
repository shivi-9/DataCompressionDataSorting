...
int main()
{
    ...
    //std::vector<std::bitset<32>> encodedData = encodeLZ77(data, 8, 4);
    std::vector<int> data = read_data("./Workload/Workload40KB/100_1.txt");
    ...
    ofstream outfile("./LZ77/EncodedData/40KB/100_1.txt");
    for (std::bitset<32> token : encodedData) {
        outfile << token << " ";
    }
    std::cout << std::endl;
